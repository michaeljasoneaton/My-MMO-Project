...
public partial struct Skill
{
    ...
    public bool CheckDistance(Entity caster, out Vector2 destination) => data.CheckDistance(caster, level, out destination);
    public void Apply(Entity caster, Vector2 direction) => data.Apply(caster, level, direction);
